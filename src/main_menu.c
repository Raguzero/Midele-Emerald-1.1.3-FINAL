...
static void Task_NewGameBirchSpeechSub_InitPokeBall(u8 taskId)
{
    ...
    CreatePokeballSpriteToReleaseMon(spriteId, gSprites[spriteId].oam.paletteNum, 112, 58, 0, 0, 32, 0x0000FFFF, SPECIES_WOOLOO);
...
static void Task_NewGameBirchSpeech_StartNamingScreen(u8 taskId)
{
    if (!gPaletteFade.active)
    {
        ...
        NewGameBirchSpeech_SetDefaultPlayerName(Random() % ARRAY_COUNT(gMalePresetNames));
        DestroyTask(taskId);
...
{
    return CreatePicSprite2(SPECIES_WOOLOO, SHINY_ODDS, 0, 1, a, b, 14, -1);
}
