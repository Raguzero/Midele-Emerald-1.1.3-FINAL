        ...
        while (topX < botX)
        {
            ...
            case ENCOUNTER_TYPE_LAND:
                if (MetatileBehavior_IsLandWildEncounter(tileBehaviour))
                {
                    ...
                    { // inside (cave)
                        if (IsZCoordMismatchAt(gEventObjects[gPlayerAvatar.spriteId].currentElevation, topX, topY))
...
static bool8 TryStartHiddenMonFieldEffect(u8 environment, u8 xSize, u8 ySize, bool8 smallScan)
{
    ...
    if (DexNavPickTile_NTries(environment, xSize, ySize, smallScan, MAX_DEXNAV_TRIES))
    {
        ...
        case ENCOUNTER_TYPE_LAND:
            ...
            break;
        case ENCOUNTER_TYPE_WATER:
            fldEffId = FLDEFF_SPLASHING_WATER;
            break;
        default:
            return FALSE;
        }
...
static bool8 DexNav_LoadGraphics(void)
{
    ...
    case 0:
        ...
        decompress_and_copy_tile_data_to_vram(1, sDexNavGuiTiles, 0, 0, 0);
...
static void TryDrawIconInSlot(u16 species, s16 x, s16 y)
{
    ...
    if (species == SPECIES_NONE || species > NUM_SPECIES)
        ...
        CreateMonIcon(species, SpriteCB_MonIcon, x, y, 0, 0xFFFFFFFF, formId);
}
...
bool8 TryFindHiddenPokemon(void)
{
    ...
    if ((*stepPtr) == 0 && (Random() % 100 < HIDDEN_MON_SEARCH_RATE))
    {
        ...
        EventObjectGetLocalIdAndMap(&gEventObjects[gPlayerAvatar.eventObjectId], &gFieldEffectArguments[0], &gFieldEffectArguments[1], &gFieldEffectArguments[2]);
...
{
    u16 chain = gSaveBlock1Ptr->dexNavChain;
    if (chain > 40)
        chain = 40;

    // Si la cadena es de 40 o más, hay 1/100 de forzar que sea shiny
    // En caso contrario, hay n/4000 de forzar que sea shiny si la cadena es de n
    return (Random() % 4000) < chain;
}
