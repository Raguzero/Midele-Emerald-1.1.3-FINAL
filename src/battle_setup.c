...
EWRAM_DATA const u8 *sTrainerADefeatSpeech = NULL;
EWRAM_DATA const u8 *sTrainerBDefeatSpeech = NULL;
...
const struct RematchTrainer gRematchTable[REMATCH_TABLE_ENTRIES] =
{
    ...
    [REMATCH_JUAN] = REMATCH(TRAINER_JUAN_1, TRAINER_JUAN_2, TRAINER_JUAN_3, TRAINER_JUAN_3, TRAINER_JUAN_3, SOOTOPOLIS_CITY),
...
void StartLegendDogsBattle(void)
{
    ScriptContext2_Enable();
    gMain.savedCallback = CB2_EndScriptedWildBattle;
    gBattleTypeFlags = BATTLE_TYPE_LEGENDARY;

    CreateBattleStartTask(B_TRANSITION_BLACKHOLE1 , MUS_SUIKUN); 
...
static u8 GetTrainerBattleTransition(void)
{
	...
	if (gTrainers[gTrainerBattleOpponent_A].trainerClass == TRAINER_CLASS_PKMN_TRAINER_MIDELE
        ...
        || gTrainers[gTrainerBattleOpponent_A].trainerClass == TRAINER_CLASS_RICH_ALPHA
...
static void CB2_GiveStarter(void)
{
    ...
    *GetVarPointer(VAR_STARTER_MON) = gSpecialVar_Result;
	...
	GiveMonWithLevelAndAbilityNum(starterMon, 5, ITEM_ORAN_BERRY, 0);
    ResetTasks();
    PlayBattleBGM();
...
static bool32 UpdateRandomTrainerRematches(const struct RematchTrainer *table, u16 mapGroup, u16 mapNum)
{
    ...
    for (i = 0; i <= REMATCH_SPECIAL_TRAINER_START; i++)
    {
        if (table[i].mapGroup == mapGroup && table[i].mapNum == mapNum && !sub_80B1D94(i))
        {
            ...
            else if (FlagGet(FLAG_MATCH_CALL_REGISTERED + i))  // 31% chance of getting a rematch.
            {
